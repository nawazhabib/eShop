...
public class JDBCTemplate {
    ...
    private void updateQuery(String query, Object... parameteres){
        try(var connection = dataSource.getConnection(); var statement = connection.prepareStatement(query)){
            addParameters(statement, parameteres);
    ...
    public void query(String query, ThrowableConsumer<ResultSet> consumer){
        try(var connection = dataSource.getConnection(); var statement = connection.prepareStatement(query)){
            consumer.acceptZ(statement.executeQuery());
        } catch (SQLException e){
    ...
    public long executeInsertQuery(String query, Object... parameters){
        try(var connection = dataSource.getConnection(); var preparedStatement = connection.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)){
            addParameters(preparedStatement, parameters);

            final int affectedRows = preparedStatement.executeUpdate();

            if (affectedRows == 0){
                throw new SQLException("Creating user failed, no rows affectd.");
            }
            try(ResultSet keys = preparedStatement.getGeneratedKeys()){
                ...
                throw new new RuntimeException(e);
            }
        }

        public <E> List<E> queryObject(String query, ObjectRowMapper<E> objectRowMapper){
            try(var connection = dataSource.getConnection(); var statement = connection.prepareStatement(query)){
        ...
        public List<E> queryForObject(String query, Object param, ObjectRowMapper<E> objectRowMapper){
            try(var connection = dataSource.getConnection(); var statement = connection.prepareStatement(query)){
        ...
        private void addParameters(PreparedStatement preparedStatement, Object[] parameters) throws SQLException{
            ...
            for (Object parameter : parameters){
                if (parameter instanceof String){
                    ...
                    preparedStatement.setTimestamp(idx, imestamp.valueOf((LocalDateTime) parameter));
